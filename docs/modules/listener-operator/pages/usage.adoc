...
The Listener Operator is used differently depending on whether you're using Stackable data platform operators or building custom applications.

== For Stackable Data Platform Users

If you're using Stackable data platform operators (NiFi, Kafka, Druid, etc.), the Listener Operator works automatically behind the scenes when you specify a `listenerClass` in your cluster configuration.
...
.Connection methods:
- **From within Kubernetes**: Use the service name (`nifi-node.default.svc.cluster.local`)
